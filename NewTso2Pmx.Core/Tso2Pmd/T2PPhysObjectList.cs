...
namespace Tso2Pmd
{
    public class T2PPhysObjectList
    {
        ...
        private short GetRequiredBoneIDByName(string name)
        {
            short boneId = GetBoneIDByName(name);
            if (boneId < 0)
            {
        ...
        private sbyte GetRequiredBodyIDByName(string name)
        {
            sbyte bodyId = GetBodyIDByName(name);
            if (bodyId < 0)
            {
                throw new KeyNotFoundException("Rigid body not found: " + name);
            }
        ...
        private sbyte GetRequiredJointIDByName(string name)
        {
            sbyte jointId = GetJointIDByName(name);
            if (jointId < 0)
            {
                throw new KeyNotFoundException("Joint not found: " + name);
            }

            return jointId;
        }

        private PMD_Bone GetBoneAt(int boneIndex)
        {
            if (boneIndex < 0 || boneIndex >= nodes.Count)
            {
                throw new KeyNotFoundException("Bone index out of range: " + boneIndex);
            }

            return nodes[boneIndex];
        }
        // 名前からボーンを得る
        public PMD_Bone GetBoneByName(string name)
        {
            return nodes[GetRequiredBoneIDByName(name)];
        }
        // 名前から剛体を得る
        public PMD_RBody GetBodyByName(string name)
        {
            return bodies[GetRequiredBodyIDByName(name)];
        }
        // 名前からジョイントを得る
        public PMD_Joint GetJointByName(string name)
        {
            return joints[GetRequiredJointIDByName(name)];
        }
        ...
        {
            MakeBodyFromBoneEnd(GetRequiredBoneIDByName(bone_name));
        }

        // 指定したボーン位置に合わせた剛体を生成
        public void MakeBodyFromBoneEnd(short bone_num)
        {
            PMD_Bone bone = GetBoneAt(bone_num);
            PMD_RBody rbody = new PMD_RBody();

            rbody.name = bone.name; // 諸データ：名称 // 頭
            rbody.rel_bone_id = bone_num; // 諸データ：関連ボーン番号 // 03 00 == 3 // 頭
            rbody.position = bone.position;
        ...
        {
            PMD_Bone bone = GetBoneByName(bone_name);
            if (bone.TailName == null)
            {
                throw new KeyNotFoundException("Bone tail not found: " + bone_name);
            }

            PMD_Bone tailBone = GetBoneByName(bone.TailName);
        ...
        public void MakeBodyFromTwoVector(string bone_name, Vector3 v1, Vector3 v2)
        {
            ...
            rbody.rel_bone_id = GetRequiredBoneIDByName(bone_name); // 諸データ：関連ボーン番号 // 03 00 == 3 // 頭
        ...
        {
            MakeJointFromBone(GetRequiredBoneIDByName(bone_name));
        }
        ...
        public void MakeJointFromBone(int bone_num)
        {
            ...
            {
                throw new KeyNotFoundException("Parent bone not found: " + bone.name);
            }

            PMD_Bone parentBone = GetBoneByName(bone.ParentName);
            PMD_Joint joint = new PMD_Joint();

            joint.name = parentBone.name
                + "-" + bone.name; // 諸データ：名称 // 右髪1

            joint.rbody_a_id = GetRequiredBodyIDByName(parentBone.name); // 諸データ：剛体A
            joint.rbody_b_id = GetRequiredBodyIDByName(bone.name); // 諸データ：剛体B
        ...
        {
            MakeJointFromTwoBones(GetRequiredBoneIDByName(bone_name1), GetRequiredBoneIDByName(bone_name2));
        }

        // 指定した２つのボーンにフィットさせている剛体をジョイントする
        public void MakeJointFromTwoBones(int bone_num1, int bone_num2)
        {
            PMD_Bone bone1 = GetBoneAt(bone_num1);
            PMD_Bone bone2 = GetBoneAt(bone_num2);
            PMD_Joint joint = new PMD_Joint();

            // 諸データ：名称 // 右髪1
            joint.name = bone1.name + "-" + bone2.name;
            joint.name_en = bone1.name_en + "-" + bone2.name_en;
            
            joint.rbody_a_id = GetRequiredBodyIDByName(bone1.name); // 諸データ：剛体A
            joint.rbody_b_id = GetRequiredBodyIDByName(bone2.name); // 諸データ：剛体B

            // 諸データ：位置(x, y, z) // 諸データ：位置合せでも設定可
            joint.position.X = 0.5f * (bone1.position.X + bone2.position.X);
            joint.position.Y = 0.5f * (bone1.position.Y + bone2.position.Y);
            joint.position.Z = 0.5f * (bone1.position.Z + bone2.position.Z);
